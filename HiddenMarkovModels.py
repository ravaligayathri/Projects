...
class HMM():
    ...
    def __init__(self, transmission_prob, emission_prob, obs=None):
        ...
        - emission_prob: an (m * n) 2-D numpy array, where m is the number of
        ...
        self.obs = obs
        self.emiss_ref = {}
        # Modifying vector to 3 elements for warm state
        self.forward_final = [0 , 0 , 0]
        self.backward_final = [0 , 0 , 0]
        self.state_probs = []
    ...
    def get_gamma(self):
        '''
        Calculates the gamma matrix for hot,cold and warm weathers.
        '''
        # modifying gamma function to add warm state making it 3d array where 0,1,2 are cold, warm and hot states respectively
        self.gamma = [[0, 0, 0] for i in range(len(self.observations))]
        for i in range(len(self.observations)):
            self.gamma[i][0] = (float(self.forward[0][i] * self.backward[0][i]) /
                                float(self.forward[0][i] * self.backward[0][i] +
                                self.forward[1][i] * self.backward[1][i]+self.forward[2][i] * self.backward[2][i]))
            self.gamma[i][1] = (float(self.forward[1][i] * self.backward[1][i]) /
                                float(self.forward[0][i] * self.backward[0][i] +
                                self.forward[1][i] * self.backward[1][i])+self.forward[2][i] * self.backward[2][i])
            #matrix change
            self.gamma[i][2] = (float(self.forward[2][i] * self.backward[2][i]) /
                                float(self.forward[0][i] * self.backward[0][i] +
                                self.forward[1][i] * self.backward[1][i]+self.forward[2][i] * self.backward[2][i]))
    ...
    def calculate_psi(self, t, i, j):
        ...
        # modifying to include warm state where 0,1,2 are cold, warm and hot states respectively.
        denom = float(self.forward[0][i] * self.backward[0][i] + self.forward[1][i] * self.backward[1][i]+self.forward[2][i] * self.backward[2][i])
...
if __name__ == '__main__':
    ...
    #modified emission and transmission arrays to indicate another state warm making it 3d array.
    emission = np.array([[0.7, 0.1 , 0], [0.2, 0.7, 0.4], [0.1, 0.2, 0.6]])
    transmission = np.array([ [0, 0, 0, 0, 0], [0.2, 0.6, 0.3, 0.1, 0], [0.5, 0.2,0.1, 0.5, 0], [0.3, 0.1, 0.5,0.3, 0],[0, 0.1, 0.1,0.1, 0]])
    transmission_saved = np.array([ [0, 0, 0, 0, 0], [0.2, 0.6, 0.3, 0.1, 0], [0.5, 0.2,0.1, 0.5, 0], [0.3, 0.1, 0.5,0.3, 0],[0, 0.1, 0.1,0.1, 0]])
    ...
    print()
    print("Model transmission probabilities before traiining:\n", transmission_saved)
    print("Col defs:")
    print("P(C/W/H|Start), P(C/W/H|H),P(C/W/H|W), P(C/W/H|C), P(C/W/H|Stop)")
    print()
    print("Model transmission probabilities after training:\n{}".format(model.transmission_prob))
    print()
